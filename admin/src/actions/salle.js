...
export const editSalle = (formData, id) => async (dispatch) => {
  try {
    const config = {
      headers: {
        "Content-Type": "application/json",
    ...
    const res = await axios.put(`/api/classroom/${id}`, formData, config);
...
export const getCurrentSalle = (id) => async (dispatch) => {
  dispatch({ type: SALLE_LOADING });
